...
class FightSide:
    def __init__(self, enemy: Optional[str] = None, health: Optional[int] = None, data: Optional[dict] = None):
        if data:
...
class Player:
    def __init__(self, data: Union[dict, str], from_redis: bool = False, need_deserialize: bool = False):
        ...
        if from_redis:
            ...
            self.fight_side = FightSide(data=data.get("fight_side")) if data.get("fight_side") else None
        else:
    ...
    def add_fight_side(self, enemy: "Player"):
        self.fight_side = FightSide(enemy=enemy.uuid, health=self.health)
